...
* Changed default QoS normal from max 500 cpus to 512, to allow 8 full nodes for a user as the default. Other QoS unchanged.
...
**By default, jobs run with the normal QoS (currently, up to 500 concurrent CPUs), and no changes are required for such workloads**. If you need access to a special QoS (such as larger number of CPUs), you must explicitly request it in your Slurm script, for example:
```
#SBATCH --qos=burstcpu
```
...
* Partition **math-alderaan-gpu** (run time 7 days max) is limited to 3000G memory per user to help prevent one user from blocking both 2000G 2GPU nodes for a week. 
