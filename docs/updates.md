...
Alderaan is a High Performance Computing (HPC) facility, operating on the edge of what the hardware and cooling allow. When CPUs get too hot, they are slowed down adaptively to avoid heat damage. When the disks in storage nodes get too hot, jobs are periodically suspended to allow pending disk activity to resolve anf to reduce overall thermal load. When the datacenter temperature is too high, all nodes shut down automatically.
