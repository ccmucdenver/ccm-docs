 ...
 * in a web browser to [Jupyterhub](jupyterhub.md) to **[https://math-alderaan.ucdenver.pvt](https://math-alderaan.ucdenver.pvt)**, or
 * by [Remote Desktop](remote_desktop.md) (use Windows App on macOS) to **math-alderaan**, or
 ...
 * Submit the script to the SLURM scheduler. See [examples](examples.md) for details.
 ...
 * if you run software that needs [modules](modules.md) loaded first: add `module load module_name` in your job script. 

* If you need to run on compute nodes interactively, not as a script, you can:
  * use [Jupyterhub](jupyterhub.md), which provides multiple terminal and notebook windows with dedicated resources
...
* If you need to run Python with custom environments, use [Conda](conda.md).
